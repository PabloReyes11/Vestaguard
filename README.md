...
- **`Servidor/` (Backend e IA):** Contiene el código Python de ejecución local. Aloja el servidor de Inteligencia Artificial (OpenCV DNN Caffe SSD) encargado de procesar las imágenes entrantes, el puente bidireccional de MQTT a Google Firebase (`firebase_vestaguard.py`), y la configuración del broker Mosquitto local.
